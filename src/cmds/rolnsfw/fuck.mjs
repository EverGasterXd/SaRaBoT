...
class Command {
    ...
    async run() {
        let img = await nekoapi.NSFW.rolplayNsfw.fuck()
     
        const mencion = this.message.mentions.members.first()
    
    const embed = new MessageEmbed()
     .setDescription(`${this.message.author.username} se cogió a ${this.message.mentions.members.first()} `)  
     .setColor("RANDOM")
     .setImage(img.url)
     ...
     if (!this.message.channel.nsfw) {
      ...
      return await this.message.reply({content:"tengo una idea mejor NO LO VAS HACER",
      allowedMentions: {
       repliedUser: false
 }
}).then((msg) => {
