...
class Command {
    ...
    async run() {
        let img = await nekoapi.NSFW.rolplayNsfw.spank()

        const mencion = this.message.mentions.members.first()

    const embed = new MessageEmbed()
   .setDescription(`${this.message.author.username} nalgeo a ${this.message.mentions.members.first()} `)
     .setColor("RANDOM")
     .setImage(img.url)
     ...
     if (!this.message.channel.nsfw) {
      ...
      return await this.message.reply({content:"estoy pensando que no estas muy bien",
      allowedMentions: {
       repliedUser: false
 }
}).then((msg) => {
