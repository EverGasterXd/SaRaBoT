...
class Command {
    ...
    async run() {
        let img = await nekoapi.NSFW.rolplayNsfw.kuni()

        const mencion = this.message.mentions.members.first()
    
        const embed = new MessageEmbed()
          .setDescription(`${this.message.author.username} le la lambio vagina a  ${this.message.mentions.members.first()} `)
          .setColor("RANDOM")
          .setImage(img.url)
        ...
        if (!this.message.channel.nsfw) {
          ...
          return await this.message.reply({content:"Por que lo harias :flushed:",
          allowedMentions: {
           repliedUser: false
     }
    }).then((msg) => {
