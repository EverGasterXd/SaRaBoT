...
class Command {
    ...
    async run() {
        let img = await nekoapi.NSFW.rolplayNsfw.cum()
      
       const mencion = this.message.mentions.members.first()

    const embed = new MessageEmbed()
     .setDescription(`${this.message.author.username} se vino en ${this.message.mentions.members.first()}`)
     .setColor("RANDOM")
     .setImage(img.url)
      ...
      return await this.message.reply({content:"porque te quieres dar semen a ti mismo",
      allowedMentions: {
       repliedUser: false
 }
}).then((msg) => {
