...
class Command {
    ...
    async run() {
        let img = await nekoapi.NSFW.rolplayNsfw.boobjob()
    
        const mencion = this.message.mentions.members.first()
   
      const embed = new MessageEmbed()
       .setDescription(`${this.message.author.username} le hizo una rusa a ${this.message.mentions.members.first()} `)
       .setColor("RANDOM")
       .setImage(img.url)
       ...
       if (!this.message.channel.nsfw) {
        ...
        return await this.message.reply({content:"te vas a roper la columna en el intento",
        allowedMentions: {
         repliedUser: false
   }
}).then((msg) => {
