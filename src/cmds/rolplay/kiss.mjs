...
class Command {
    ...
    async run() {
        let img = await nekoapi.SFW.action_1.kiss()

        const mencion = this.message.mentions.members.first()

      const embed = new MessageEmbed()
      .setDescription(`${this.message.author.username} beso a ${this.message.mentions.members.first()} `)
      .setColor("RANDOM")
      .setImage(img.url)
       ...
       return await this.message.reply({content: "autobeso no se puede bro",
       allowedMentions: {
        repliedUser: false
  }
}).then((msg) => {
