...
class Command {
    ...
    async run() {
        let img = await nekoapi.SFW.action_1.shot()

        const mencion = this.message.mentions.members.first()
       
       const embed = new MessageEmbed()
         .setDescription(`${this.message.author.username} disparo a ${this.message.mentions.members.first()}`)
       if (!mencion) {
         ...
         return await this.message.reply({content:"nooo lo hagas", 
         allowedMentions: {
           repliedUser: false
     }
    }).then((msg) => {
