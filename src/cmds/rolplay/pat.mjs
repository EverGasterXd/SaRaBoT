...
class Command {
    ...
    async run() {
        let img = await nekoapi.SFW.action_1.pat()

        const mencion = this.message.mentions.members.first()
    
        const embed = new MessageEmbed()
          .setDescription(`${this.message.author.username} acaricio a ${this.message.mentions.members.first()} `)
          .setColor("RANDOM")
          .setImage(img.url)
          ...
          if (!mencion) {
            ...
            return await this.message.reply({content: "no te cacaricies solo",
            allowedMentions: {
             repliedUser: false
       }
     }).then((msg) => {
